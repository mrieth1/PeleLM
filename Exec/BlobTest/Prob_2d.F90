...
! ::: -----------------------------------------------------------
  ...
  subroutine init_data(level,time,lo,hi,nscal, &
      ...
      use PeleLM_2D, only: pphys_RHOfromPTY, pphys_HMIXfromTY, pphys_HfromT, pphys_TfromHY
      ...
      integer dimloc(2) 
      data dimloc / 1,  1 /
      REAL_T H_dummy_in(1:nspec)
      REAL_T H_dummy_co(1:nspec)
      REAL_T T_dummy(1)

      REAL_T,parameter :: errMAX = BL_REAL_E(7.8,-12)
      integer,parameter :: NiterMAX = 20
      REAL_T  res(0:NiterMAX-1)
      integer statuts

      T_dummy = T_in
      call pphys_HfromT(dimloc,dimloc, &
          H_dummy_in(1:nspec), DIMARG(dimloc), DIMARG(dimloc), &
          T_dummy(1),          DIMARG(dimloc), DIMARG(dimloc) )

      T_dummy = T_co
      call pphys_HfromT(dimloc,dimloc, &
          H_dummy_co(1:nspec), DIMARG(dimloc), DIMARG(dimloc), & 
          T_dummy(1),          DIMARG(dimloc), DIMARG(dimloc) )
      ...
      do j = lo(2), hi(2)
	    ...
	    !eta = 0.5d0*(1.d0 - TANH(2.d0*(sqrt(x**2+y**2))/blobr))            
	    eta = 0.5d0*(1.d0 - TANH(2.d0*(sqrt(x**2+y**2)-blobr)/(.1d0*blobr)))

            vel(i,j,1) = V_in
            vel(i,j,2) = 0.0d0

	    scal(i,j,RhoH) = eta * H_dummy_in(2) + (1.d0-eta) * H_dummy_co(2)
      ...
      do j = lo(2), hi(2)
         do i = lo(1), hi(1)
            ...
            scal(i,j,Trac) = scal(i,j,Trac)*scal(i,j,Density)
         enddo
      enddo
